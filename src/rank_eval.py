"""
Reproduce Figure 6 of the paper: the average predicted relative rank of the
ground-truth extrusion among all candidate extrusions, per method.

For every test sequence with processed ground truth (written by
train_preprocess.py), each step contributes one datum: generate all proposals
for the step's zone graph, rank them with each method, find the GT extrusion
by zone-set hash, and record rank / candidate_count. Lower is better.

Paper reference values (Fig 6, Fusion360 test set): Ours Net 0.036,
Ours Heur 0.070, Random 0.486.

Usage (needs the trained checkpoints in --train_output):
